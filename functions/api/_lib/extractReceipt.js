...
### קטגוריות — קיימות מול חדשות:
קטגוריות קיימות (העדף אותן כשהפריט מתאים): ${allowedCategories.join(' | ')}

**חשוב — אל תכפה הכול למזון!** העסק קונה גם דלק, ציוד, שירותים ועוד. אם פריט/קבלה אינו משתייך באופן ברור לאף קטגוריה קיימת, **צור קטגוריה ראשית חדשה ומתאימה** (category_l1) וסמן is_new_category=true. אסור לסווג פריט לקטגוריה לא-מתאימה (לדוגמה: דלק שייך ל"תחבורה ודלק", לא ל"מוצרי מזון ומכולת").
דוגמאות לקטגוריות חדשות נפוצות: "תחבורה ודלק", "מסעדות ואירוח", "ציוד משרדי", "שירותים ותקשורת", "אחזקה ותיקונים", "מסים ואגרות", "שונות".

### כללי סיווג למוצרי מזון (לעסק קפה):
